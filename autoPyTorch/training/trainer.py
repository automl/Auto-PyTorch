...
from autoPyTorch.components.networks.image.mobilenet import MobileNet, ParallelMobileNet
...
class Trainer(object):
    def __init__(self, loss_computation, model, criterion, budget, optimizer, scheduler, budget_type, device, images_to_plot=0, checkpoint_path=None, config_id=None):
        ...
        #if torch.cuda.device_count() > 1:
        #    print("TRAINER: Found " + str(torch.cuda.device_count()) + " devices")
        #    if isinstance(model, MobileNet):
        #        #self.model = DDP(ParallelMobileNet(model, n_gpus=torch.cuda.device_count()))
        #        self.model = ParallelMobileNet(model, n_gpus=torch.cuda.device_count())
        #    else:
        #        model = nn.DataParallel(model)
        #        self.model = model.to(self.device)
        #else:
