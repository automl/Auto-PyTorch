...
class _LearnedEntityEmbedding(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any], num_categories_per_col: np.ndarray, num_numerical_features: int):
        """
        Args:
        ...
        categorical_features = self.num_categories_per_col > 0

        self.num_categorical_features = self.num_categories_per_col[categorical_features]

        self.embed_features = [num_in >= 2 for num_in in
                               self.num_categories_per_col]
        self.num_output_dimensions = [ceil(config["dimension_reduction_" + str(i)] * num_in) for i, num_in in
                                           enumerate(self.num_categorical_features)]                 
                                           
        self.num_output_dimensions.extend([0] * num_numerical_features)

        self.num_output_dimensions = [num_out if embed else 1 for num_out, embed in
