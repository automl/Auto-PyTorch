...
class TransferAndRefit2(PipelineNode):
    def fit(self, pipeline_config, autonet):
        if pipeline_config["transfer_dataset_2"] is not None:

            logging.getLogger('benchmark').info("Start refit on transfer dataset 2")
            ...
            directory = os.path.join(autonet_config['result_logger_dir'], 'transfer_2')
            ...
            # Get datasets
            transfer_dataset_train = pipeline_config["transfer_dataset_2"]
            transfer_dataset_test = pipeline_config["transfer_dataset_test_2"]
            ...
            # write as json
            with open(os.path.join(directory, "transfer_summary_2.json"), "w") as f:
                json.dump(summary, f)
    ...
    def get_pipeline_config_options(self):
        options = [
            ConfigOption("transfer_dataset_2", default=None, type=str),
            ConfigOption("transfer_dataset_test_2", default=None, type=str)
        ]
        return options
